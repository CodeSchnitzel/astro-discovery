...
> **Date:** March 25, 2026
> **Hardware:** Raspberry Pi Zero 2 W, Debian 13 (Trixie), aarch64, 416 MB RAM, 29 GB SD
...
- **Flask web server** — serves the plate solving UI on port 443 (HTTPS)
...
2. iPhone/phone connects to WiFi network **"AstroDiscovery"** (password: `stargazer`)
3. Open Safari: **https://10.42.0.1**
4. Drag/drop or tap to upload photo, or use the Camera tab to solve directly from a connected DSLR
5. Results appear in ~80-100 seconds on Pi Zero 2 W
...
| ASTAP CLI | `apt install astap-cli` (Debian 13 repos, arm64) |
...
|-----------|-------------|---------------|
| RAW (NEF) conversion | ~20-26 sec | ~2 sec |
| Astrometry.net solve | ~60-80 sec | ~5-10 sec |
| ASTAP with hint | ~5-7 sec | ~1-2 sec |
| Total end-to-end | ~80-100 sec | ~10-15 sec |

### RAM Notes (416 MB actual)
- Astrometry.net works well — it memory-maps index files, which is RAM-friendly
- ASTAP struggles with blind solves due to RAM constraints, but works with coordinate hints from chaining
- Uploads use `/var/lib/astro-discovery/uploads` (SD card), not `/tmp` (which is a 209 MB tmpfs)

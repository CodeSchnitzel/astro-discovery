# AstroDiscovery Plate Solving Station - User Manual

AstroDiscovery runs on two platforms:

| Platform | OS | Access |
|----------|-----|--------|
| **StarFinder VM** (Proxmox VMID 103) | Debian 12.12 | `https://starfinder` or `https://10.1.10.196` |
| **Raspberry Pi Zero 2 W** | Debian 13 (Trixie) | WiFi: `https://10.42.0.1` / USB: `https://10.42.1.1` |
...
### StarFinder VM
- **SSH:** `ssh root@starfinder` or `ssh root@10.1.10.196`
- **Web UI:** Open `https://starfinder` or `https://10.1.10.196` in any browser
- **Proxmox Console:** https://deepthought:8006 > VM 103 > Console

### Raspberry Pi Zero 2 W (Field Use)
- **WiFi hotspot:** Connect to SSID **"AstroDiscovery"** (password: **stargazer**), open `https://10.42.0.1`
- **USB gadget:** Plug Pi into laptop USB, open `https://10.42.1.1`
- **SSH:** `ssh root@10.42.0.1` (WiFi) or `ssh root@10.42.1.1` (USB)

### HTTPS Certificate (iOS GPS)
The Pi uses a self-signed SSL certificate. For first-time setup on iPhone:
1. Visit `https://<pi-ip>/cert` in Safari to download the profile
...
### Option D: Camera Direct (Pi Zero only)
Plug your DSLR into the Pi via USB. Open the **Camera** tab in the web UI to browse and solve photos directly from the camera's memory cards.

### Option E: VirtualBox Shared Folders (field use)
...
## 7. Taking AstroDiscovery Into the Field (VirtualBox)

The StarFinder VM was built for portability — all hardware settings are VirtualBox-compatible.
...
| Spec | Value |
|------|-------|
| Name | StarFinder (VM) / astro-discovery (Pi) |
| Proxmox VMID | 103 |
...
| **Web UI not loading (VM)** | Check service: `systemctl status starfinder-web`. Restart: `systemctl restart starfinder-web`. |
| **Web UI not loading (Pi)** | Check service: `systemctl status astro-discovery`. Restart: `systemctl restart astro-discovery`. |
| **Camera not detected (Pi)** | Reconnect USB cable. Check: `gphoto2 --auto-detect`. Kill any competing process: `pkill gphoto2`. |
| **GPS denied on iPhone** | Install the SSL cert profile: visit `https://<ip>/cert`. Enable in Settings > Certificate Trust Settings. |
| **Pi WiFi hotspot not appearing** | Check: `nmcli connection show`. Restart: `nmcli connection up AstroDiscovery-AP`. |

---

*AstroDiscovery Plate Solving Station — Built March 2026*
